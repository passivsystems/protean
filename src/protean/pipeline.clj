...
(defn project-test [id host port]
  (assoc json :body
         (txco/js-> (txt/testy-analysis-> id ((keyword id) @state) host port))))
...
(defn project-docs [id host port]
  (txdocs/project-template id (txan/analysis-> id ((keyword id) @state) host port)))
